...
def save_checkpoint(unet, optimizer, lr_scheduler, global_step, epoch, args, accelerator, best_loss, val_loss, accumulation_steps):
    ...
    if not accelerator.is_main_process:
        return best_loss
    step = int(global_step * accumulation_steps)  # Adjust step for accumulation
    # Save checkpoint
    ...
    # Save as model.pt if validation loss improved
    if val_loss is not None and (best_loss is None or val_loss < best_loss):
        best_loss = val_loss
        best_file_path = os.path.join(args.save_dir, "model.pt")
        accelerator.save({
            "model": unwrapped_unet.state_dict(),
            "optimizer": optimizer.state_dict(),
            "lr_scheduler": lr_scheduler.state_dict(),
...
def train(unet, train_loader, val_loader, autoencoder, tokenizer, text_encoder, 
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            if global_step % args.val_step == 0 and global_step > 0:
                ...
                print(f"Saving checkpoint at global step {global_step} due to validation")
            ...
            if args.max_step is not None and global_step >= args.max_step:
                print(f"Reached maximum step {args.max_step}. Saving checkpoint and stopping training.")
    ...
    best_loss = save_checkpoint(unet, optimizer, lr_scheduler, global_step, epoch, args, accelerator, best_loss, None, accumulation_steps)
    print(f"Final checkpoint saved at global step {global_step}")
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        close_logging(writer, args)
...
def parse_args():
    ...
    parser.add_argument('--save-step', type=int, default=500, help='Steps between saving checkpoints')
    ...
    parser.add_argument('--max_step', type=int, default=None, help='Maximum number of training steps')
    ...
    parser.add_argument('--resume_from_checkpoint', type=str, default=None, help='Path to a specific checkpoint file to resume training from')
